...
public class TC_QuickMenu extends GenericFunctions {
...
//	@Parameters({"quickAdd","TestName"})
	...
	/*	methodName = Thread.currentThread().getStackTrace()[1].getMethodName().toString();
		...
		for(int i=0; i <testCaseIDs.length; i++) {
			...
			qm=PageFactory.initElements(driver, QuickMenuObjects.class);
			//qm.verifyQuickAdd(driver, test, data1,testCaseIDs[i], timeout);
